...
function GetTextToSave($post)
{
	...
	$tempHourToCheck = date("H", strtotime('2 hour'));//echo "$hour</br>";
	if ($tempHourToCheck < "6") 
	{
		$date = date("d.m.Y", strtotime('- 6 hour'));
		$time = date("H:i", strtotime('2 hour'));//echo "Before 06:00";
    } 
	else
	{
		$date = date("d.m.Y");
		$time = date("H:i", strtotime('2 hour'));//echo "After 06:00";
	}
